...
class MonitoringMessage(object):
    class Info:
        ALL_COINS_NOT_REACHED_EXPECTED_PROFIT = (
            "거래 가능한 코인 중 모든 코인이 최소 수익 값에 도달하지 못했습니다."
        )
        COIN_NOT_REACHED_EXPECTED_PROFIT = (
            "해당 코인의 예상 차익이 최소 입력 차익보다 낮습니다. 코인: {}, 예상 차익: {}, 입력 수익: {}"
        )
    ...
    class Debug:
        ...
        GET_ERROR_MESSAGE_IN_COMPARE = (
            "get error message in _compare_orderbook. error_message={}"
        )
        BALANCE_NOT_FOUND = "{}, has not {} in currency balance."
        EXPECTED_PROFIT = (
            "expected profit is not enough to reach setting's profit. coin:{}, {} < {}"
        )
        ...
        SET_PROFIT_DICT = (
            "btc profit has reached the min_profit. min_profit={}, profit_dict={}"
        )
...
class WithdrawalMessage(object):
    class Info(object):
        MANUAL_INFO = "입금 거래소({}), " "출금 코인={} 출금 코인의 수량={}, 출금 주소={}, 출금 태그={}"
