...
from DiffTrader.settings.test_settings import (
    UPBIT_TEST_INFORMATION,
    BINANCE_TEST_INFORMATION,
)
...
class Monitoring(object):
        ...
        while True:
            if not DEBUG:
                ...
                if primary_contents and secondary_contents:
                    ...
                    if isinstance(primary_information, int) or isinstance(
                        secondary_information, int
                    ):
                        time.sleep(1)
                        continue
                    latest_primary_information = json.loads(
                        primary_information, cls=DecimalDecoder
                    )
                    latest_secondary_information = json.loads(
                        secondary_information, cls=DecimalDecoder
                    )

                    logging.debug(
                        Msg.Debug.GET_INFORMATION.format(
                            latest_primary_information, latest_secondary_information
                        )
                    )

                elif (
                    not latest_primary_information and not latest_secondary_information
                ):
                ...
                # test code
                latest_primary_information = json.loads(
                    UPBIT_TEST_INFORMATION, cls=DecimalDecoder
                )
                latest_secondary_information = json.loads(
                    BINANCE_TEST_INFORMATION, cls=DecimalDecoder
                )
            tradable_symbol_list = self._get_tradable_symbols(
                latest_primary_information, latest_secondary_information
            )
            ...
            arbitrage_data = self.get_arbitrage_primary_secondary(
            ...
            profit_dict = self._get_max_profit(
                primary,
                secondary,
    ...
    def _get_tradable_symbols(self, primary_information, secondary_information):
        ...
        logging.debug(
            CMsg.entrance_with_parameter(
                self._get_tradable_symbols, (primary_information, secondary_information)
            )
        )
        primary_deposit_symbols = primary_information["deposit"].keys()
        secondary_deposit_symbols = secondary_information["deposit"].keys()
        deposit_intersection = set(primary_deposit_symbols).intersection(
            secondary_deposit_symbols
        )
    ...
    def get_arbitrage_primary_secondary(self, primary, secondary, tradable_symbol_list):
        ...
        if success:
            ...
            for sai_symbol in tradable_symbol_list:
                if (
                    sai_symbol not in primary_result.data
                    or sai_symbol not in secondary_result.data
                ):
                ...
                primary_to_secondary_result = __bid_ask_calculator(
                    secondary_bids, primary_asks
                )
                ...
                secondary_to_primary_result = __bid_ask_calculator(
                    primary_bids, secondary_asks
                )
            ...
            error_message = "\n".join(
                [primary_result.message, secondary_result.message]
            )
    ...
    def _get_max_profit(
        ...
        profit_dict = dict()
        for exchange_running_type in [
            TraderConsts.PRIMARY_TO_SECONDARY,
            TraderConsts.SECONDARY_TO_PRIMARY,
        ]:
            for sai_symbol in arbitrage_data["calculated_symbol_list"]:
                ...
                if not primary_information["balance"].get(coin):
                    logging.warning(
                        Msg.Debug.BALANCE_NOT_FOUND.format(
                            self._primary_str, sai_symbol
                        )
                    )
                    time.sleep(10)
                    continue
                    ...
                    logging.warning(
                        Msg.Debug.BALANCE_NOT_FOUND.format(
                            self._secondary_str, sai_symbol
                        )
                    )
                    time.sleep(10)
                    continue

                expect_profit_percent = arbitrage_data["expected_profit_dict"][
                    exchange_running_type
                ][sai_symbol]

                if not DEBUG and expect_profit_percent < self._min_profit:
                    logging.info(
                        Msg.Info.COIN_NOT_REACHED_EXPECTED_PROFIT.format(
                            coin, expect_profit_percent, self._min_profit
                        )
                    )
                    continue
                ...
                if exchange_running_type == TraderConsts.PRIMARY_TO_SECONDARY:
                    expectation_data = {
                        "from": __expectation_setter(
                            primary, primary_information, arbitrage_data["primary"]
                        ),
                        "to": __expectation_setter(
                            secondary,
                            secondary_information,
                            arbitrage_data["secondary"],
                        ),
                    }
                else:
                    expectation_data = {
                ...
                logging.debug(
                    Msg.Debug.TRADABLE_INFO.format(
                        tradable_btc,
                        coin_amount,
                        sell_coin_amount,
                        btc_profit,
                ...
                if refresh_profit_dict:
                    profit_dict = {
                        ...
                        "additional_information": {
                            ...
                            "from_exchange_str": expectation_data["from"][
                                "exchange"
                            ].name,
    ...
    def _get_expectation(self, expectation_data, expected_profit_percent, sai_symbol):
        ...
        def __find_min_balance(btc_amount, coin_amount, to_exchange_coin_bid):
            ...
            tradable_btc = (
                btc_amount if btc_amount < coin_to_btc_price else coin_to_btc_price
            )

            return tradable_btc

        def __get_real_difference(
            from_information, to_information, expected_profit_percent, market
        ):
            # transaction fee에 대한 검증은 get_expectation 에서 진행
            from_trading_fee_percent = (
                1 - from_information["trading_fee"][market]
            ) ** from_information["fee_count"]
            to_trading_fee_percent = (
                1 - to_information["trading_fee"][market]
            ) ** to_information["fee_count"]

            real_diff = (
                (1 + expected_profit_percent)
                * from_trading_fee_percent
                * to_trading_fee_percent
            ) - 1
        ...
        real_difference = __get_real_difference(
            from_["information"], to_["information"], expected_profit_percent, market
        )
